...
*     Prepare time-stepping information      
      ...
      !prepare RT3D output averaging
      sum_tracker = 0 !rtb
      rt_year = 1 !rtb
      open(130000,file='rt3d_averages') !rtb
      write(130000,*) 'Annual averages for each cell'
      write(130000,*) 'Year 1, Year 2, Year 3, etc...'
      allocate(cnew_total(ncol,nrow,nlay,ncomp))
      allocate(cnew_avg(ncol,nrow,nlay,ncomp,50))
      cnew_total = 0.
      cnew_agv = 0.
