...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: profile_z, vadose_z, fract_below, fract_above !rtb
      real, dimension (:), allocatable :: layer_seepage !rtb
...
!!      aird(j) = 0.
      ...
      allocate(layer_seepage(sol_nly(j))) !rtb
      layer_seepage = 0. !rtb
      profile_z = 0. !rtb
     ...
     &                    + sptno2concs(isp))  
       ...
       !! determine gravity drained water in layer
        ...
        if (sw_excess > 1.e-5) then
          ...
          !store sepday in an array (to be used below) !rtb
          layer_seepage(j1) = sepday
        ...
        !rtb (SWAT-MODFLOW) (01-05-2017)
        !Determine if the water table is in the soil profile. If so:
        !  1. saturate all soil layers below
        !  2. find layer from which recharge should come 
        if(mf_active.eq.1) then
          
          if(mf_shallow_dynamics.eq.1) then
          
          !cumulative thickness of the soil profile
          profile_z = profile_z + sol_z(j1,j)

          !check to see if the water table is in the current layer
          vadose_z = vadose_thick_hru(j)
          if(vadose_thick_hru(j) .le. profile_z) then
            
            !calculate fraction of the layer above and below the water table
            fract_below = (profile_z - vadose_z) / sol_z(j1,j)
            fract_above = 1 - fract_below

            !determine layer from which recharge percolates
            !  - if water table is near the bottom of the soil profile, recharge
            !    should come from this layer
            !  - if water table is near the top or middle of the soil profile, recharge
            !    should come from the above layer
            if(fract_above .le. 0.7) then
              if(j1.eq.1) then !if top layer of soil profile
                sepbtm(j) = sepbtm(j) + layer_seepage(j1) !current layer
                rch_layer(j) = j1
              else
                sepbtm(j) = sepbtm(j) + layer_seepage(j1-1) !layer above
                rch_layer(j) = j1-1
              endif
            else
              sepbtm(j) = sepbtm(j) + layer_seepage(j1) !current layer
            ...
            !set soil below the water table to the saturation level
            do jj=j1,sol_nly(j)
              if(jj.eq.j1) then
                !partial saturation, based on the position of the water table
                sol_st(jj,j) = sol_st(jj,j) + 
     &                         ((sol_ul(jj,j)-sol_st(jj,j))*fract_below)
              else
                !full saturation
                sol_st(jj,j) = sol_ul(jj,j)
              endif
            enddo

            !set fractions for lateral flow and tile drain flow
            latq(j) = latq(j) + (latlyr * fract_below)
            qtile = qtile + (lyrtile * fract_below)
            flat(j1,j) = (latlyr*fract_below) + (lyrtile*fract_below)
            sol_prk(j1,j) = sol_prk(j1,j) + sepday
            if (latq(j) < 1.e-6) latq(j) = 0.
            if (qtile < 1.e-6) qtile = 0.
            if (flat(j1,j) < 1.e-6) flat(j1,j) = 0.

            goto 10 !exit the loop - all calculations are complete
 ...
 10   if (ievent > 0) then
      ...
      if (sol_tmp(2,j) > 0.) then
        ...
        !if MODFLOW is active, then shallow water table depth comes from MODFLOW
        !rtb 02-09-2017
        if(mf_active.eq.1) then
          vadose_z = vadose_thick_hru(j) !thickness of the unsaturated zone
          wt_shall = dep_imp(j) - vadose_z !saturated thickness above the impervious layer
          d = dep_imp(j) - ddrain(j) !vertical distance between drain and the impervious layer
        else !!drainmod calculations for wt depth
        wt_shall = dep_imp(j)
        por_air = 0.5
...
!       Updated water table depth D.Moriasi 4/8/2014
        ...
        !! drainmod wt_shall equations   10/23/2006
        endif !rtb (SWAT-MODFLOW)
        
        !compute tile drainage flow
        if (ddrain(j) > 0.) then
