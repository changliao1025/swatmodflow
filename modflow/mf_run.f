...
C     Specify which modules are to be used
      ...
      USE LMTMODULE
      use mf_rt_link !tcw
      use smrt_parm, only:num_MF_obs,MF_obs,mf_obs_flag
      
      INCLUDE 'mf_openspec.inc'

*     Declare variables
      integer   num_ts,t,kkstp
      ...
      !Read inputs unless SM or SMRT linkage is running in which case mf_read has already been called !tcw
      readMFinput = .true. !tcw
      MF_leapyr = 1 !tcw
      call mfruntop%fire() !tcw
     ...
     &                            IGRID)    
      ...
      !Added these events to handle passing in SWAT information to MODFLOW  !tcw
      IF(IUNIT(4).GT.0) CALL ONRIVPKG%FIRE() !tcw
      IF(IUNIT(8).GT.0) CALL ONRECHPKG%FIRE() !tcw
      IF(IUNIT(55).GT.0) CALL ONUZFPKG%FIRE() !tcw
      
      !If UZF-RT3D is used, read in stress information for UZF-RT3D as well, rtb
      CALL OnRT3Dinit%FIRE() !rtb !tcw
      
      !determine the time step duration -----------------------------------------------------------
      
      !convert the mf_interval value (in days) to MODFLOW's current units, tcw
      !mfDayLength(1) = mf_interval !tcw
      !call units(mfDayLength, 4, ITMUNI, 1, 1, MF_leapyr) !tcw
      ...
      !Determine if the duration of each flow time step (As specified in the MODFLOW .dis file) is less than 1 day or more, tcw
      !ts_length = PERLEN(kper) / NSTP(kper) !tcw
      
      !num_ts = NSTP(kper)
      !DELT = ts_length
      
      !if(ts_length.gt.mfDayLength(1)) then !rtb
      !  DELT = mfDayLength(1)
      !  num_ts = 1
      !endif
...
C--SWM: SWAP POINTERS FOR LMT DATA TO CURRENT GRID
        !populate the variables - if SM or SMRT is used, this is handled by smrt_read_river2grid subroutine, !tcw
        if(readMFinput)then !tcw
          CALL SLMT7PNT(IGRID)
        end if !tcw
 ...
 9876   FORMAT(/1X,'SAVING SATURATED THICKNESS AND FLOW TERMS ON UNIT',
   ...
   87         FORMAT(1X,'FAILURE TO MEET SOLVER CONVERGENCE CRITERIA',/
     &       1X,'BUDGET PERCENT DISCREPANCY IS',F10.4)
        ...
        !If solute transport is simulated, call UZF-RT3D subroutines to simulate reactive transport
        !for the current flow time step. !rtb
        CALL OnRT3D%FIRE() !rtb !tcw
...
*     for the current stress period.
      ...
      if(mf_obs_flag.eq.1) then
        if(num_MF_obs.gt.0) then
          ...
          enddo
          write(30021,100) (obs_heads(i),i=1,num_MF_obs)
        endif
      endif
