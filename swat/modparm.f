      ...
      !Events and subscriber indices for SWAT packages for easy decoupled model integration !aqd
      use modevent !aqd
      type(event), target :: OnInit !aqd, tcw
      type(event), target :: normOutUpdate !aqd, tcw
      type(event), target :: OnCommand19 !aqd, tcw
      type(event), target :: OnCommandFinished !aqd, tcw
      type(event), target :: OnGroundWater !tcw
      type(event), target :: OnSoilTempCalc !tcw
      type(event), target :: OnCropGrowthCalc !tcw
      type(event), target :: OnRouteInitialize !aqd

      !SWAT variable made global for linkage with MODFLOW !tcw
      real, dimension (:), allocatable :: etremain !tcw
      real, dimension (:), allocatable :: rttlc !tcw
      integer, dimension (:), allocatable :: rch_layer !rtb
      logical :: normOut !tcw

      ! Turn off GW computation (aqd, tcw)
      logical :: computegw, computenutr, initializeRCHloss !tcw
      
      !SWAT variable made global for linkage with UZF-RT3D !tcw
      real, dimension (:), allocatable :: perc_no3_conc !tcw
