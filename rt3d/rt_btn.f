...
C       Open up file to output XY (surface contour) concentration at each output time
        OPEN(ICONC+I,FILE='rt_output/1concentration/CONC'//COMP,
     &     STATUS='UNKNOWN')

C       Open up file to output XZ (cross section) concentration at each output time
        OPEN(IXSECT+I,FILE='rt_output/1concentration/XZ'//COMP,
     &       STATUS='UNKNOWN')

C       If the species is mobile:
        IF (PHASE(INDEX).EQ.1) THEN
C         Cumulative source/sink mass            
          OPEN(ISSMASS+INDEX,FILE='rt_output/2ssmass/SSMASS_CUMUL_'
     &         //COMP,STATUS='UNKNOWN')

C         Source/sink mass for each time step
          OPEN(ISSMASS+50+INDEX,FILE='rt_output/2ssmass/SSMASS_'
     &         //COMP,STATUS='UNKNOWN') 

C         Leaching at output times)
		  OPEN(ILCH+INDEX,FILE='rt_output/6leach/LCH_' 
     &	         //COMP,STATUS='UNKNOWN')
     
C         Total mass in saturated vs. unsaturated zones
          IF(TRNOP(7)) THEN !VST package is used
            OPEN(IMASSOUT+I,FILE='rt_output/7mass/mass'
     &           //COMP,STATUS='UNKNOWN')
            WRITE(IMASSOUT+I,*) 
     &     'Total mass in saturated and unsaturated zones (kg)'
          ENDIF     
        ENDIF
      ENDDO   
...
C		Create and open up files for observation cell concentration output        
		DO INDEX=1,NCOMP
		  FLNAME = 'rt_output/3observations/OBS'//SPECIES(INDEX)
...
C        WRITE(IOUT,1064) IMAS,NPRMAS
        EXT='mas'
C		Create and open up files for mass balance output        
		DO INDEX=1,NCOMP
			FLNAME = 'rt_output/4massbalance/MB'//SPECIES(INDEX)
			OPEN(IMAS+INDEX, FILE=FLNAME, STATUS='UNKNOWN')
			WRITE(IMAS+INDEX,*) 'Unit of Mass:',MUNIT
			WRITE(IMAS+INDEX,1067)  'TIME2','INITIAL','PREVIOUS',
     &                              'CURRENT','DM_TOTAL','DM_CURRENT',
     &                              'SOURCE','SINK','DM_SS','% ERROR',
     &                              '% ERROR TOTAL'
C			WRITE(IMAS+INDEX,1068) TUNIT,MUNIT,MUNIT,MUNIT,MUNIT
		ENDDO
C       Create a rt_global mass balance output file (will have cumulative 
C       mass balance error written for each species at each output time).		
	  OPEN(IMAS+49, FILE='rt_output/4massbalance/MB_Global')
	  WRITE(IMAS+49,*) 'Cumulative mass balance errors'
	  WRITE(IMAS+49,*) !Skip line		
      ELSE
        WRITE(IOUT,1065)
      ENDIF
...
C     9. Write a one-line mass budget summary for the specie
      ...
      WRITE(IMAS+ICOMP,1012) TIME2,INITMASS,OLDMASS,NEWMASS,DM_TOTAL,
     &                       DM_TS,ABS(SOURCE_TOT),ABS(SINK_TOT),DM_SS,
     &                       ERROR2_TS(ICOMP),ERROR2(ICOMP) 
...
C       Leaching at observation cells (only for mobile species)          
        IF (PHASE(ICOMP).EQ.1) THEN
          WRITE(ILCHOBS+ICOMP,1081) TIME2,(LEACH
     &     (LOCOBS(2,N),LOCOBS(1,N),LOCOBS(3,N),ICOMP),N=1,NOBS)
        ENDIF
      ENDIF
      ENDIF
 
C     Write out first layer concentration in array format
      open(146001,file='rt3d_no3grid') !rtb
      do i=1,nrow
        write(146001,1081) (CNEW(j,i,1,1),j=1,ncol)
      enddo
      write(146001,*)
...
C			  For current cell, write out species concentration 			    
     ...
     &                                MC(J,I,K) !ASCII
		  ...
		  ENDIF
	  ENDDO
C       Row by Row (XZ plane)     
        TEXT = '   CONC.'      
        DO I=1,NROW
C         For the current row, write out information
          IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN  
            WRITE(IXSECT+ICOMP,1019) NTRANS,rt_kstp,rt_kper,TIME2,TEXT,
     &                              'ROW',I
          ENDIF
          IF (WRITEFLAG.EQ.1 .OR. WRITEFLAG.EQ.2) THEN
           WRITE(IXSBIN+ICOMP) NTRANS,rt_kstp,rt_kper,TIME2,TEXT,'ROW',I
          ENDIF
C         BINARY output
          IF (WRITEFLAG.EQ.1 .OR. WRITEFLAG.EQ.2) THEN
		    WRITE(IXSBIN+ICOMP) ((CNEW(J,I,K,ICOMP),K=1,NLAY),J=1,NCOL)
		  ENDIF
          IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN 
C           For the current row, write out values for every column and layer          
            DO J=1,NCOL
              DO K=1,NLAY
C               For current cell, write out species concentration
                if(trnop(7)) then
                WRITE(IXSECT+ICOMP,*) XBC(J),ZBC(J,I,K),
     &                                   CNEW(J,I,K,ICOMP),MC(J,I,K)
                else
                WRITE(IXSECT+ICOMP,*) XBC(J),ZBC(J,I,K),
     &                                CNEW(J,I,K,ICOMP)
                endif
          ...
          ENDIF
        ENDDO
C       Leaching, layer by layer (only for mobile species) 
        IF (PHASE(ICOMP).EQ.1) THEN !Only proceed if it is a mobile species
	    IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN 
	      WRITE(ILCH+ICOMP,*) 'LEACHING AT TIME',TIME2
	    ENDIF
	    IF (WRITEFLAG.EQ.1 .OR. WRITEFLAG.EQ.2) THEN 
	      WRITE(ILCHBIN+ICOMP) 'LEACHING AT TIME',TIME2
	    ENDIF
	    DO K=1,NLAY
C		    For the current layer, write out information
		    IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN 
		      WRITE(ILCH+ICOMP,*) 'LAYER',K
		    ENDIF
		    IF (WRITEFLAG.EQ.1 .OR. WRITEFLAG.EQ.2) THEN 
		      WRITE(ILCHBIN+ICOMP) 'LAYER',K
		    ENDIF
C			For current cell, write out species concentration
C           'ASCII
			IF (WRITEFLAG.EQ.0 .OR. WRITEFLAG.EQ.2) THEN 		    
		    DO I=1, NROW
		      DO J=1, NCOL
			    WRITE(ILCH+ICOMP,1016) XBC(J),YBC(I),ZBC(J,I,K),
     &                                 LEACH(J,I,K,ICOMP),
     &                                 TLEACH(J,I,K,ICOMP)
              ENDDO
            ENDDO
...
C           Binary
			IF (WRITEFLAG.EQ.1 .OR. WRITEFLAG.EQ.2) THEN 
			  WRITE(ILCHBIN+ICOMP) 
     &                     ((LEACH(J,I,K,ICOMP),J=1,NCOL),I=1,NROW)
            ENDIF            
	    ENDDO
	  ENDIF
      ENDIF
...
C           Report in kg (to avoid large numbers)            
		...
		ENDDO
	ENDDO 
	WRITE(IMASSOUT+ICOMP,*) TIME2,MASS_SAT,MASS_UNSAT  
	ENDIF
	ENDIF     
...
C     Print out the cumulative mass balance error in the rt_global mass balance
C     output file.
      IF (ICOMP.EQ.1) THEN !If first species, write time step
        WRITE(IMAS+49,*) 'Current Time is:',TIME2
      ENDIF
      WRITE(IMAS+49,1099) SPECIES(ICOMP),ERROR2(ICOMP)
 1099 FORMAT(A15,2X,F12.5)
      IF (ICOMP.EQ.NCOMP) THEN
        WRITE(IMAS+49,*) !Skip a line before the next output time
      ENDIF
